...
export default {
  ...
  methods: {
    ...
    async deleteItemConfirm() {
      try {
        await axios.delete(`http://localhost:3000/alunos/${this.editedItem.id}`);
    ...
    async save() {
      if (this.editedIndex > -1) {
        ...
        try {
          await axios.put(`http://localhost:3000/alunos/${this.editedItem.id}`, this.editedItem);
        ...
        try {
          const response = await axios.post('http://localhost:3000/alunos', this.editedItem);
    ...
    async logout() {
      try {
        ...
        this.$router.push('/#'); // Redirecionar para a página de login após logout
      } catch (error) {
